...
function ENT:Zombie_CustomOnPreInitialize()
	self.Model = {"models/vj_piv/specials/shikari/Fast.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
	...
	self.HasLeapAttack = true -- Should the SNPC have a leap attack?
	self.AnimTbl_LeapAttack = {"vjseq_leapstrike"} -- Leap Attack Animations
	...
	self.LeapToMeleeDistance = 150 -- How close does it have to be until it uses melee?
	self.NextLeapAttackTime = 5 -- How much time until it can use a leap attack?
	self.TimeUntilLeapAttackVelocity = 0.2 -- How much time until it runs the velocity code?
	self.LeapAttackVelocityForward = 500 -- How much forward force should it apply?
	self.LeapAttackVelocityUp = 200 -- How much upward force should it apply?
	self.LeapAttackDamage = 0
	self.LeapAttackDamageDistance = 0 -- How far does the damage go?
	self.TimeUntilLeapAttackDamage = false

	self.TorsoEntity = "npc_vj_piv_shikari_torso"

	self.CanFlinch = 1
	self.AnimTbl_Flinch = {ACT_FLINCH_PHYSICS}
	self.SoundTbl_LeapAttackJump = {"vj_piv/shikari/leap1.wav"}
...
function ENT:Zombie_GiveVoice()
	self.SoundTbl_Alert = {"vj_piv/shikari/fz_alert_close1.wav","vj_piv/shikari/fz_alert_far1.wav","vj_piv/shikari/fz_alert_far2.wav"}
	self.SoundTbl_BeforeMeleeAttack = {"npc/fast_zombie/leap1.wav"}
	self.SoundTbl_Pain = {"vj_piv/shikari/pain1.wav","vj_piv/shikari/pain2.wav","vj_piv/shikari/pain3.wav","vj_piv/shikari/pain4.wav"}
	self.SoundTbl_Death = {"vj_piv/shikari/die1.wav","vj_piv/shikari/die2.wav","vj_piv/shikari/die3.wav","vj_piv/shikari/die4.wav"}
...
function ENT:Zombie_CustomOnTakeDamage_PostDamage(dmginfo,hitgroup)
	if GetConVar("vj_piv_cripple"):GetInt() == 1 then  -- if the convars not on don't run this
